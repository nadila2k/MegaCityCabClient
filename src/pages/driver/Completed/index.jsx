...
const Completed = () => {
  ...
  // Fetch completed bookings from API on component mount
  useEffect(() => {
    const fetchCompletedBookings = async () => {
      try {
        ...
        setCompletedBookings(response.data.data); // Assuming data is an array of bookings
        setLoading(false);
      } catch (err) {
  ...
  const getStatusIcon = (status) => {
    switch (status) {
      ...
      case "COMPLETED":
        return <CheckCircleIcon sx={{ color: "#388e3c" }} />; // Green for COMPLETED
      default:
        return null;
    }
  };
  ...
  if (loading) {
    return (
      <Box sx={{ p: 3 }}>
        <PageTitle title="Completed Bookings" />
